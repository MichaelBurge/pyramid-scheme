  ...
  (λ (vm i reads)
    (parameterize ([ *warnings?* #f])
      ...
      (printf "~a\t~a\t~a\t~v\t~v\t~v\t~v\t~v\t~v\t~v\t~v\t~v\t~a\n"
              ...
              (shrink-evm i)
              (map (λ ([ ptr : EthWord ]) (shrink-value (vm-value vm ptr)))
                   (vm-exec-stack vm))
              (shrink-value (vm-register vm 'val))
              (shrink-value (vm-register vm 'continue))
              (shrink-value (vm-register vm 'proc))
              (shrink-value (vm-register vm 'argl))
...
(define (vm-vector vm x)
  (let* ([ addr (read-memory-word vm (+ x #x40) WORD)]
    ...
    (v-vector
     (for/list ([ i (in-range addr end WORD)])
       (vm-value vm i)))))
